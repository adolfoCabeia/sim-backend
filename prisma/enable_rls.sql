...
ALTER TABLE processos_anexos ENABLE ROW LEVEL SECURITY;
ALTER TABLE processos_anexos FORCE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS tenant_isolation_processos_anexos ON processos_anexos;
CREATE POLICY tenant_isolation_processos_anexos ON processos_anexos
  USING (
    "processoId" IN (
...
ALTER TABLE ofertas_antecipacao ENABLE ROW LEVEL SECURITY;
ALTER TABLE ofertas_antecipacao FORCE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS tenant_isolation_ofertas_antecipacao ON ofertas_antecipacao;
CREATE POLICY tenant_isolation_ofertas_antecipacao ON ofertas_antecipacao
  USING ("municipioId" = current_setting('app.current_municipio_id', true)::text);
...
-- ─── departamentos (@@map confirmado — sem municipioId próprio, escopo via direcao) ───
...
CREATE POLICY tenant_isolation_departamentos ON departamentos
  USING (
    "direcaoId" IN (
      SELECT id FROM "Direcao"   -- ⚠️ confirmar @@map de Direcao — ver nota abaixo
      WHERE "municipioId" = current_setting('app.current_municipio_id', true)::text
    )
  );
