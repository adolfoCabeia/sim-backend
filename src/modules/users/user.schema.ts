...
const ESTADOS_UTILIZADOR = ["ACTIVA", "SUSPENSA", "BLOQUEADA", "PENDENTE_VALIDACAO"] as const;
...
export const criarUtilizadorSchema = z.object({
  ...
  estado: z.enum(ESTADOS_UTILIZADOR).default("PENDENTE_VALIDACAO"),

  direcaoSigla: z.enum(DIRECAO_SIGLAS).optional(),
  departamentoNome: z.string().min(1).optional(),
...
export const editarUtilizadorSchema = z.object({
  ...
  direcaoSigla: z.enum(DIRECAO_SIGLAS).nullable().optional(),
  departamentoNome: z.string().min(1).nullable().optional(),
  areaResponsabilidade: z.enum(AREAS_RESPONSABILIDADE).nullable().optional(),
