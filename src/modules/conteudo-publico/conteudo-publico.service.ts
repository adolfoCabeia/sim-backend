...
export async function criarConteudoPublico(params: {
  ...
  return withTenantTransaction(params.municipioId, async (tx) => {
    const existente = await tx.conteudoPublico.findUnique({
      where: { municipioId_chave: { municipioId: params.municipioId, chave: params.input.chave } },
      select: { id: true },
    });
    if (existente) {
      throw new ChaveJaExisteError(`Já existe um conteúdo com a chave "${params.input.chave}" neste município.`);
    }
    ...
    return tx.conteudoPublico.create({
      data: {
        ...
        ...(params.input.resumo !== undefined && { resumo: params.input.resumo }),
      },
    });
  });
}
...
export async function editarConteudoPublico(params: {
  ...
  return withTenantTransaction(params.municipioId, async (tx) => {
    ...
    return tx.conteudoPublico.update({
      ...
      data: {
        ...(params.input.titulo !== undefined && { titulo: params.input.titulo }),
        ...(params.input.resumo !== undefined && { resumo: params.input.resumo }),
        ...(params.input.corpo !== undefined && { corpo: params.input.corpo }),
        ...(params.input.categoria !== undefined && { categoria: params.input.categoria }),
      },
    });
  });
}
  ...
  return withTenantTransaction(params.municipioId, async (tx) => {
    ...
    return tx.conteudoPublico.update({
      ...
      data: {
        ...
        gruposComAcesso: params.input.estadoPublicacao === "RESTRITO" ? (params.input.gruposComAcesso ?? []) : [],
...
export async function despublicarConteudoPublico(params: { municipioId: string; conteudoId: string }) {
  return withTenantTransaction(params.municipioId, async (tx) => {
    await obterConteudoOuFalhar(tx, params.conteudoId);
    return tx.conteudoPublico.update({
      where: { id: params.conteudoId },
...
export async function eliminarConteudoPublico(params: { municipioId: string; conteudoId: string }): Promise<void> {
...
export async function listarConteudosPublicosAdmin(params: { municipioId: string; query: ListarConteudosPublicosQuery }) {
  return withTenantTransaction(params.municipioId, async (tx) => {
    const where: Prisma.ConteudoPublicoWhereInput = {
      ...(params.query.categoria !== undefined && { categoria: params.query.categoria }),
      ...(params.query.estadoPublicacao !== undefined && { estadoPublicacao: params.query.estadoPublicacao }),
    };
...
export async function obterConteudoPublicoAdmin(params: { municipioId: string; conteudoId: string }) {
  return withTenantTransaction(params.municipioId, (tx) => obterConteudoOuFalhar(tx, params.conteudoId));
}

export async function listarConteudosPublicos(params: { municipioId: string; query: ListarConteudosPublicosPublicoQuery }) {
  return withTenantTransaction(params.municipioId, async (tx) => {
    const where: Prisma.ConteudoPublicoWhereInput = {
      estadoPublicacao: { in: ["PUBLICADO", "PUBLICADO_PARCIAL"] },
      ...(params.query.categoria !== undefined && { categoria: params.query.categoria }),
    };
    const [items, total] = await Promise.all([
      tx.conteudoPublico.findMany({
...
export async function obterConteudoPublicoPorChave(params: { municipioId: string; chave: string }) {
  return withTenantTransaction(params.municipioId, async (tx) => {
    const conteudo = await tx.conteudoPublico.findUnique({
      where: { municipioId_chave: { municipioId: params.municipioId, chave: params.chave } },
    });
    if (!conteudo || !["PUBLICADO", "PUBLICADO_PARCIAL"].includes(conteudo.estadoPublicacao)) {
      throw new ConteudoPublicoNaoEncontradoError("Conteúdo não encontrado.");
    }
...
function ocultarCorpoSeParcial<T extends { estadoPublicacao: string; corpo: string }>(conteudo: T): T {
