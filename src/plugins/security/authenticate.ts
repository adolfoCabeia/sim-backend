...
const ROTAS_PERMITIDAS_COM_TROCA_PASSWORD_PENDENTE: Array<{ method: string; path: string }> = [
  { method: "GET", path: "/utilizadores/me" },
  { method: "POST", path: "/utilizadores/me/change-password" },
];
...
export const authenticatePlugin = fp(async (app) => {
  app.decorate("authenticate", async (request, reply) => {
    try {
      ...
      request.log.warn({ err, hasAuthHeader: !!request.headers.authorization }, "jwtVerify falhou");
    ...
    if (request.user.deveTrocarPassword) {
      const permitido = ROTAS_PERMITIDAS_COM_TROCA_PASSWORD_PENDENTE.some(
        (rota) => rota.method === request.method && rota.path === caminhoActual
      );
      if (!permitido) {
    ...
    const { permitido: dentroDaJanelaDeAcesso } = await verificarJanelaDeAcesso({
